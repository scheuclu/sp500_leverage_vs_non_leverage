...
for date, price_non_leverage, price_leverage, rel_non_leverage, rel_leverage in zip(times, non_leverage, leverage, rel_diff_non_leverage, rel_diff_leverage):
    ...
    if NUM_SHARES_LEVERAGE>0 and datetime.fromisoformat(date)-datetime.fromisoformat(date_last_trade)>timedelta(minutes=10):
        ...
        print(NUM_SHARES_NON_LEVERAGE * price_non_leverage + NUM_SHARES_LEVERAGE * price_leverage,
              NUM_SHARES_NON_LEVERAGE, NUM_SHARES_LEVERAGE)
    else:
...
trace1=go.Scatter(
        ...
        name='non-leveraged',
        text=day,
        marker=dict(color='black', showscale=True,),  # 👈 important: makes the colorbar visible
        mode='lines')
trace2=go.Scatter(
        x=times,
        y=leverage,
